 ...
 */
public class Game implements iGameResource {

	private GameMap map;
	private  List<HZObject> objects;
	private  List<Unit> units;
	private  List<Effect> effects;
	private  List<HZWidget> overlays;
	private  List<Effect> newEffects;
	private  List<Player> players;
	private  List<Base> bases;
	private  Map<Player, UnitOrder> unitOrders;
	private  Map<Player, Mech> mechs;
	private  Player player;
	private ManagerOverlay managerOverlay;
	...
	public void orderUnit(Player player, String unitName) {
		unitOrders.put(player,
				new UnitOrder(unitName, unitFactory.getTimeToBuild(unitName)));
		player.setUnitOrder(unitOrders.get(player));
	}

	public Unit pickupOrder(Player player) {
		UnitOrder order = unitOrders.remove(player);
		return resManager.getUnitFactory().buildUnit(player,
				order.getUnitName());
	}
	...
	public Unit getUnitAt(float x, float y, float tolerance) {
		for (Unit unit : units) {
			if (!(unit instanceof Mech)
					&& GameMath.dist(unit.getPos().x, unit.getPos().y, x, y) < (unit
							.getRadius() + tolerance)) {
				return unit;
			}
		}
...
//		for (int i = 0; i < 50; i++) {
//			Unit unit = res.getUnitFactory().buildUnit(
//					(i % 2 == 0) ? players[0] : players[1], "Tank");
